<#-------------------------------------------------------------------------------------------------
  ZipCodeLookup - Export-Documentation.ps1
...
# Before running, you need to have the most current version of ZipCodeLookup loaded in memory
Remove-Module ZipCodeLookup -ErrorAction SilentlyContinue
Import-Module ./ZipCodeLookup
...
New-MarkdownHelp -Module ZipCodeLookup `
...
# Now loop over each one fixing the issue
  foreach ($f in $FilePath)
  {
    ...
    foreach ($line in $contents)
    {
      ...
      # If we are at the input or output area, turn on the flag to begin checking
      ...
      $lookFor = '[https://github.com/arcanecode/ZipCodeLookup/blob/master/Documentation/'
      ...
      $replacements = @{'Author: Robert C Cain | @ArcaneCode | arcane@arcanetc.com' = 'Author: Robert C Cain | [@ArcaneCode](https://twitter.com/arcanecode) | arcane@arcanetc.com'
                        '[https://github.com/arcanecode/ZipCodeLookup](https://github.com/arcanecode/ZipCodeLookup)' = '[ZipCodeLookup on GitHub](https://github.com/arcanecode/ZipCodeLookup)'
    ...
    {
      if ($line -eq '[ZipCodeLookup on GitHub](https://github.com/arcanecode/ZipCodeLookup)')
      {
